...
const REUSE_EXISTING_GENERATED_FILES = false;
...
function authorizeGmailDrafting() {
  ...
  GmailApp.createDraft(
    ...
    'Apps Script Gmail Draft Authorization Test',
    'This draft was created to authorize Gmail draft permissions. You may delete it.'
  );
...
function preflightGmailDraftAuthorization(config) {
  ...
  try {
    ...
    vLog(`  - [PREFLIGHT] [FATAL] Gmail preflight authorization failed: ${e.message}`);
    throw new Error(`Gmail preflight failed. Original error: ${e.message}`);
  }
}
...
function preflightSpreadsheetAuthorization(config) {
  ...
  try {
    ...
    vLog(`  - [PREFLIGHT] [FATAL] Spreadsheet access check failed: ${e.message}`);
    throw new Error(`Spreadsheet preflight failed. Original error: ${e.message}`);
  }
}
  ...
  try {
    ...
    threads.forEach((thread, threadIdx) => {
      ...
      messages.forEach((msg, msgIdx) => {
        ...
        const rawContent = msg.getRawContent().toLowerCase();
        ...
        for (let j = sentJobs.length - 1; j >= 0; j--) {
          ...
          for (let k = 0; k < job.emails.length; k++) {
            ...
            const hasEmailMatch = body.includes(email) || rawContent.includes(email);
            if (!hasEmailMatch) continue;

            let isMatch = false;
            // Must also contain the Job ID (or Job Title if ID is missing) to prevent false positives
            if (job.jobId && (body.includes(job.jobId) || subject.includes(job.jobId) || rawContent.includes(job.jobId))) {
              isMatch = true;
            } else if (!job.jobId && job.jobTitle && (body.includes(job.jobTitle) || subject.includes(job.jobTitle) || rawContent.includes(job.jobTitle))) {
              isMatch = true;
            }
...
/**
 * Layer 1 (Bounce Suppression) — Appends a confirmed-bounced email address to the
 * persistent Script Properties suppression store (key: 'BOUNCED_ADDRESSES').
 * Stored as a newline-delimited list. At ~30 chars/address the 9 KB per-property
 * limit accommodates ~300 entries — sufficient for months of operation.
 */
...
function isRetryableOpenRouterError(error) {
  const code = Number(error && error.responseCode);
...
function discoverApplicationEmailRoutes(metadata, config) {
  ...
  const fullResponse = askOpenRouter(prompt, config, { temperature: 0.12, top_p: 0.75 });

  vLog('[DISCOVERY] Extracting JSON structure from raw text response...');
  const parsed = parseJsonObjectFromText(fullResponse);
...
function generateLinkedInDoc(targetFolder, metadata, jobUrl) {
  ...
  const linkedInFileName = `LinkedIn_${rearPortion}`;
...
function processSingleMaterialFile(params) {
  ...
  const parsed = parseMarkdown(file.getBlob().getDataAsString());
  ...
  try {
    ...
    if (!hasLoggedFailure) {
      appendToSpreadsheetLog({
        ...
        jobTitle: getBestJobTitle(metadata),
        targetEmail: getDispatchTargetEmailsForLogging(metadata, discoveryResult).join(', '),
        emailTier: LOG_EMAIL_TIER_DISPATCH_FAILED,
...
function getNormalizedGmailAliases() {
  try {
    return GmailApp.getAliases().map(a => sanitizeEmailAddress(a)).filter(Boolean);
  } catch (e) {
    return [];
  }
}
...
function canUseGmailFromAddress(address) {
  ...
  let primary = '';
  try {
    primary = sanitizeEmailAddress(Session.getEffectiveUser().getEmail());
  } catch (e) {
    primary = '';
  }

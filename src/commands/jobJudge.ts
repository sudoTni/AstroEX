...
/**
 * Finds files matching a given single-wildcard pattern.
 * @param pattern The wildcard pattern (e.g., "./data/scraped_jobs_*.json").
 * @param baseDirectory The directory relative patterns are resolved from.
...
export async function findJobFiles(
	...
	const resolvedPrefix = path.resolve(baseDirectory, prefix);
	...
	try {
		...
		const matchingFiles = files
			.filter(
				(file) => file.startsWith(baseNamePrefix) && file.endsWith(suffix),
			)
			.sort()
			.map((file) => path.join(dir, file));
		return matchingFiles;
	} catch (error) {
		log("JobJudge", `Error finding input job files: ${error}`, "error");
		return [];
	}
}
...
function extractJobIdFromUrl(url: string, filename?: string): string {
	const hyphenatedIdMatch = url.match(/-(\d+)(?:[/?&#]|$)/);
	if (hyphenatedIdMatch) {
		return hyphenatedIdMatch[1];
	}
...
async function _processInputFiles(
	...
	if (argv["input-file"] === "./data/scraped_jobs_*.json") {
		const matchingFiles = await findJobFiles(argv["input-file"]);
...
async function createLLMRequestConfig(
	...
	maxTokens: number,
	showReasoningTokens: boolean = false,
	showResponseStream: boolean = false,
): Promise<LLMRequest> {
	...
	while (retryCount < maxRetries && !retrySuccess) {
		try {
			...
			const result = await withSpinner(
				`Waiting for LLM response for "${job.title || job.id}" (attempt ${retryCount + 1}/${maxRetries})...`,
				() => llmService.call(llmRequest),
				{
					enabled:
...
async function evaluateJobWithLLM(
	...
	const showReasoningTokens = Boolean(
		argv["show-reasoning"] || argv["show-reasoning-tokens"] || (argv as any).sr,
	);
	const showResponseStream = Boolean(
		argv["show-stream"] ||
			argv["show-stream-tokens"] ||
			(argv as any)["stream-response"] ||
			(argv as any).ss,
	);
...
export const addJobJudgeCommand = (
	...
	return yargs.command({
		...
		handler: async (
			...
			log("JobJudge", "Command started", "info", { params: argv });
			...
			try {
				...
				// Normal jobJudge execution
					...
					const _userDataDir = path.join(rootDirectory, "user_data");
					const {
						resume: myResume,
					...
					log(
						"JobJudge",
						`Reading scraped job data from: ${resolvedInputPath} `,
					);
					...
					if (argv["input-file"] === "./data/scraped_jobs_*.json") {
						...
						// Existing logic for single file or directory
						const inputPathStats = await fs.promises.stat(resolvedInputPath);
						if (inputPathStats.isDirectory()) {
							const files = await fs.promises.readdir(resolvedInputPath);
							const jsonFiles = files.filter((file) => file.endsWith(".json"));
							if (jsonFiles.length === 0) {
								throw new Error(
					...
					log(
						"JobJudge",
						`Successfully read ${processedJobs.length} scraped job records. `,
					);
					...
					for (const job of processedJobs) {
						...
						try {
							// Use one synchronous extractor for both modern hyphenated URLs
							// and older URL layouts, with a deterministic hash fallback.
							const jobId = _extractJobIdFromJob(job);
							...
							if (isMatched) {
								stats.incrementCounter("jobsSkipped", 1);
							...
							if (!job.descriptionText) {
								stats.incrementCounter("jobsMissingDescription", 1);
								stats.incrementCounter("data.recordsFiltered", 1);
								stats.recordWarning("Job has no description text", {
									jobId,
								});
								log(
									"JobJudge",
								...
								// Do not create a judged checkpoint: a later JD scrape should
								// leave this job eligible for its first evaluation.
								continue;
							}
							...
							const llmEvaluationStartedAt = performance.now();
							apiCallStarted = true;
							...
							stats.recordHistogram(
								"api.responseTime",
								performance.now() - llmEvaluationStartedAt,
							);
							...
							stats.recordSuccess("llmEvaluation", {
								jobId,
								verdict: isPassVerdict ? "pass" : "fail",
							});
							...
							if (retryCount > 0) {
								...
								stats.incrementCounter("api.retries", retryCount);
								stats.incrementCounter("api.totalCalls", retryCount);
								stats.incrementCounter("api.failedCalls", retryCount);
								if (retryCount >= 3) {
							...
							if (apiCallStarted && !apiCallSucceeded) {
							...
							// A failed scrape, API call, or file write must remain retryable.
							// Only a fully persisted evaluation above is a judged checkpoint.
							log(
								"JobJudge",
								`Leaving failed job ${jobId} eligible for retry; no jobDB judged checkpoint was written.`,
								"info",
							);
						}
					}
					...
					stats.recordSuccess("jobJudge.complete", {
						jobsLoaded: processedJobs.length,
						jobsPassed: passedJobs.length,
					});
					stats.endCollection();
				...
				const errorTyped = error as unknown;
				stats.recordError(
					errorTyped instanceof Error
						? errorTyped
						: new Error(String(errorTyped)),
				);
				log(
					"JobJudge",
				...
				log("JobJudge", "Final statistics:", "info", { summary });
			}
		},
	});
};
...
function _extractJobIdFromJob(job: JobInterface): string {
	...
	if (job.url) {
		// LinkedIn's common public URL ends in a hyphenated numeric ID.
...
async function _processAllJobFiles(
	...
	if (inputFile === "./data/scraped_jobs_*.json") {
		const matchingFiles = await findJobFiles(inputFile);
		if (matchingFiles.length === 0) {

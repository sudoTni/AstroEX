...
import { formatDuration, log } from "../utils";
...
// External prompt templates are loaded from the generic prompts directory.
...
export async function runMode1(
	...
	log("Mode1", "Started resume optimization", "info", { mode: 1 });
	...
	try {
		...
		log("Mode1", "Completed resume optimization", "info", {
			duration,
			summary,
		});
		...
		log("Mode1", "Resume optimization failed", "error", { duration, error });

		const summary = stats.endCollection();
		log("Mode1", "Final statistics", "info", { summary });
...
export async function runMode2(
	...
	log("Mode2", "Started resume optimization", "info", { mode: 2 });
	...
	try {
		...
		stats.incrementCounter("llm.calls", 1);
		stats.incrementCounter("llm.successfulCalls", 1);
		stats.recordSuccess("er44zz-mode2.complete");
		...
		log("Mode2", "Completed resume optimization", "info", {
			duration,
			summary,
		});
		...
		log("Mode2", "Resume optimization failed", "error", { duration, error });

		const summary = stats.endCollection();
		log("Mode2", "Final statistics", "info", { summary });
...
export async function runMode3(
	...
	log("Mode3", "Started job-description distillation", "info", { mode: 3 });
	...
	try {
		...
		stats.incrementCounter("llm.calls", 1);
		stats.incrementCounter("llm.successfulCalls", 1);
		stats.recordSuccess("er44zz-mode3.complete");
		...
		log("Mode3", "Completed job-description distillation", "info", {
			duration,
			summary,
		});
		...
		log("Mode3", "Job-description distillation failed", "error", {
			duration,
			error,
		});

		const summary = stats.endCollection();
		log("Mode3", "Final statistics", "info", { summary });
...
export async function runMode4(
	...
	log("Mode4", "Started job-description distillation", "info", { mode: 4 });
	...
	try {
		...
		stats.incrementCounter("llm.calls", 1);
		stats.incrementCounter("llm.successfulCalls", 1);
		stats.recordSuccess("er44zz-mode4.complete");
		...
		log("Mode4", "Completed job-description distillation", "info", {
			duration,
			summary,
		});
		...
		log("Mode4", "Job-description distillation failed", "error", {
			duration,
			error,
		});

		const summary = stats.endCollection();
		log("Mode4", "Final statistics", "info", { summary });

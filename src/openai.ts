...
export async function readExternalFile(fileName: string): Promise<string> {
	...
	const safeFilePath = path.join(rootDirectory, "user_data", fileName);
...
export async function readPromptFile(promptName: string): Promise<string> {
	const templateName = promptName.startsWith("rop_")
		? "materials.txt"
		: promptName.startsWith("jdd_")
			? "job_judge.txt"
			: undefined;

	if (!templateName) {
		throw new Error(`Unsupported prompt alias: ${promptName}`);
	}

	const rootDirectory = process.cwd();
	const filePath = path.join(rootDirectory, "prompts", templateName);
	try {
		return await fs.promises.readFile(filePath, "utf-8");
	} catch (error) {
		throw new Error(`Failed to read prompt file ${templateName}: ${error}`);
	}
}
...
export async function processBatch(
	...
	if (
		...
		log("OpenAI", "Processing title batch", "debug", {

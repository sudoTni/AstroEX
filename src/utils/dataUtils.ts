...
export function removeDuplicates(
	...
	for (const job of jobs) {
		try {
			...
			log("DataUtils", "Skipping invalid job entry", "warn", { error });
		}
	}
...
export function filterJobs(
	...
	return jobs.filter((job) => {
		try {
			...
			log("DataUtils", "Skipping invalid job during filtering", "warn", {
				error,
			});
			return false;
		}
...
export async function batchProcessJobs(
	...
	for (let i = 0; i < jobs.length; i += batchSize) {
		...
		try {
			const batchResults = await Promise.all(
				batch.map((job) =>
					processor(job).catch((error) => {
						log("DataUtils", "Failed to process job in batch", "warn", {
							error,
						});
						return null;
					}),
			...
			log("DataUtils", "Failed to process job batch", "error", { error });
			throw new AppError(

...
export function getJobDescription(
	...
	return defer(() => {
		log("LinkedIn", "Navigating to job page", "debug", { url: job.url });
		return defer(() =>
			fromPromise(
		...
		tap((response) => {
			...
			log("LinkedIn", "Received job page response", "debug", {
				status: status,
				url: job.url,
			});
		...
		// LinkedIn sends the full description in the static markup/JSON-LD. The
		// "Show more" control only changes the visual clamp, so clicking it adds
		// latency and can fail without improving the extracted content.
		switchMap(() =>
			defer(async () => {
				// Extract the job title and description text using stable selectors.
				...
				const descriptionText = await page.evaluate(() => {
					const visibleMarkup = document.querySelector(
						"div.description__text--rich div.show-more-less-html__markup",
					)?.textContent?.trim();
					if (visibleMarkup) return visibleMarkup;
					...
					for (const script of Array.from(
						...
						try {
							...
							// Ignore malformed structured data and continue with other scripts.
						}
					}
					return "";
				});

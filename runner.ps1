...
param(
    ...
    # Optional API key (takes precedence over ASTROEX_API_KEY environment variable)
    [string] $ApiKey,
...
"@
    ...
    if (-not [string]::IsNullOrWhiteSpace($ApiKey)) {
        Write-Log "Using API key provided via -ApiKey (redacted in logs)."
    }
    elseif (-not [string]::IsNullOrWhiteSpace($env:ASTROEX_API_KEY)) {
        Write-Log "Using API key from ASTROEX_API_KEY environment variable (redacted in logs)."
        $ApiKey = $env:ASTROEX_API_KEY
    }
    else {
        Write-Log "No API key provided via -ApiKey or ASTROEX_API_KEY; using default key."
    ...
    if (-not $SkipJobCloth) {
        ...
        $apiArg = if (-not [string]::IsNullOrWhiteSpace($ApiKey)) { @('--api-key', $ApiKey) } else { @() }
        Invoke-NativeChecked 'npm' (
            @( 'run', 'job:cloth', '--', '--verbose', '--log-payload', '--preset', 'jc_ds-v4-flash' ) +
            $apiArg +
            @( '-s5', '--batch', '100' )
        )
    }
    ...
    if (-not $SkipJobJudge) {
        ...
        $apiArg = if (-not [string]::IsNullOrWhiteSpace($ApiKey)) { @('--api-key', $ApiKey) } else { @() }
        Invoke-NativeChecked 'npm' (
            @( 'run', 'job:judge', '--', '--verbose', '--log-payload', '--preset', 'jep_ds-v4-flash' ) +
            $apiArg +
            @( '-s5' )
        )
    }
    ...
    if (-not $SkipMakeMaterials) {
        ...
        $apiArg = if (-not [string]::IsNullOrWhiteSpace($ApiKey)) { @('--api-key', $ApiKey) } else { @() }
        Invoke-NativeChecked 'npm' (
            @( 'run', 'makeMaterials', '--', '--verbose', '--log-payload', '--preset', 'rop_ds-v4-flash' ) +
            $apiArg +
            @( '-s5' )
        )
    }
    ...
    $Elapsed = $WorkflowStopwatch.Elapsed
    $ElapsedHours = [int][Math]::Floor($Elapsed.TotalHours)
    $ElapsedString = "{0:d2}:{1:d2}:{2:d2}.{3:d3}" -f $ElapsedHours, $Elapsed.Minutes, $Elapsed.Seconds, $Elapsed.Milliseconds
    Write-Log "AstroEX run completed successfully in $ElapsedString."
}
catch {
    if ($null -ne $WorkflowStopwatch) {
        $WorkflowStopwatch.Stop()
        $Elapsed = $WorkflowStopwatch.Elapsed
        $ElapsedHours = [int][Math]::Floor($Elapsed.TotalHours)
        $ElapsedString = "{0:d2}:{1:d2}:{2:d2}.{3:d3}" -f $ElapsedHours, $Elapsed.Minutes, $Elapsed.Seconds, $Elapsed.Milliseconds
        Write-Log "AstroEX run failed after ${ElapsedString}: $($_.Exception.Message)"
    }
    else {
